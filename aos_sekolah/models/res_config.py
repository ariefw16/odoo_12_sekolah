...
#             self.company_id.currency_id = self.currency_id
    ...
    has_default_school = fields.Boolean(readonly=True,
        default=lambda self: self._default_has_default_school())
...
#         readonly=True, help="Bank accounts as printed in the footer of each printed document")
    ...
    auto_fiscalyear = fields.Boolean(related='company_id.auto_fiscalyear', string='Auto Generate School Year', store=True)
    auto_graduate = fields.Boolean(related='company_id.auto_graduate', string='Auto Graduate for All Active Students', store=True)
...
# 
    @api.model
    def _default_has_default_school(self):
